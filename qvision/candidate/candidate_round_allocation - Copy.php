...
require '../../connect.php';
$cid=$_REQUEST['id'];
$sql=$con->query("select c.status as candidate_status,cm.*,c.*,d.*,dm.* from candidate_form_details c left join company_master cm on c.company_name=cm.id left join jobdescription_master d on c.position=d.id left join z_department_master dm on c.department=dm.id where c.id='$cid' order by c.id desc limit 1");
$fet=$sql->fetch();
$rid=$fet['resource_id'];
?>
<style>
  ...
  <div class="card-header">
   ...
   <form method="POST" enctype="multipart/form-data">
    ...
    <table class="table table-bordered">
        ...
        <td colspan="5">
		<input type="text" class="form-control" id="companys" name="companys" value="<?php echo $fet['position'];?>" readonly>
		</td>
        </tr>
	  ...
	  <td colspan="2"> 
       ...
       <tr>
		...
		<tr>
		<td>Photo:<td>
        <a href="/ssinfo1/qvision/candidate/photo/<?php echo $fet['photo'];?>" download="<?php echo $fet['photo']; ?>"><?php echo $fet['photo']; ?></a>
		</tr>
		<tr>
		<td>Resume:<td>
		<a href="/ssinfo1/qvision/candidate/uploads/<?php echo $fet['resume'];?>" download="<?php echo $fet['resume']; ?>"><?php echo $fet['resume']; ?></a>
		</tr>
