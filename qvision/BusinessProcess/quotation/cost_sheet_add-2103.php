...
$stmt = $con->prepare("SELECT a.employee as acc_manager,a.list,a.Product,a.Client,a.id as enquiry_id 
					   ...
					   ,b.id as business_id, b.mapping_id,b.name FROM Enquiry a join product_services b on (b.id = a.list) 
					   WHERE a.id='$enquiry_id'"; */
$stmt->execute(); 
$row = $stmt->fetch();
...
<div class="card card-primary">
 ...
 <div class="card-body">
 <table class="table table-striped table-bordered table-hover display nowrap"  id="new_tab" style="width:250%" border="1" style="border-collapse:collapse;margin-bottom: 0px !important;" class="table table-bordered">
	 <!--table class="table table-striped table-bordered table-hover display nowrap"  id="new_tab" style="width:100%"-->
	 	<tbody id="cost_sheett">
		  ...
		  <th>
		       ...
		       <Th style=" WIDTH: 8%;" formula="cost*qty" summary="sum" >Amount</th>
         ...
         <tr>
		     ...
		     <td>
					 <select name="item[]" id="item1" class="form-control">
					 <option disabled selected>-- Select One--</option>
						<?php
								$stmt2 = $con->prepare("SELECT name,hsn_code FROM product_master");		
								$stmt2->execute(); 
								//$count2 = $stmt2->rowCount();		
								while($row2 = $stmt2->fetch()){
				
						?>
					<option value="<?php echo $row2['name']; ?>"><?php echo $row2['name']; ?>-<?php echo $row2['hsn_code']; ?></option>
						<?php 
							}
						?>
					 </select>
    ...
    <option disabled selected>-- Select vendor --</option>
      ...
      <table id="dataTable4" width="300px" border="1" style="border-collapse:collapse;" class="table table-bordered" >
	     ...
	     <tr>
             ...
             <td colspan="4"><input type="number" style="float:left; width: 50%" class="form-control"  onchange="grandtotal()"  name="igst_per" id="igst_per" placeholder="Enter IGST Percentage"></td>
		     <td align="right">
  ...
  function calc(idx) {
	...
	document.getElementById("com_amt" + idx).value = isNaN(com_amt) ? "0.00" : com_amt.toFixed(2);
	 ...
	 var pricez = parseFloat(document.getElementById("price" + idx).value) +
      parseFloat(document.getElementById("com_amt" + idx).value);
    //alert(idx+":"+price);  
