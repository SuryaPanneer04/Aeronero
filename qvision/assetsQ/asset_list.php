...
<div  class="card card-primary">
			  ...
			  <a onclick="client_excel()"  style="float: right;padding: 4px 6px 7px 5px;position: relative;left: -3px;" data-toggle="modal" class="btn btn-danger"><i class="fa fa-plus"></i>EXCEL</a>
		    <a onclick="add_asset()"  style="float: right;padding: 4px 6px 7px 5px;position: relative;left: -21px;" data-toggle="modal" class="btn btn-dark btn"><i class="fa fa-plus"></i> ADD Asset</a> &nbsp;&nbsp;&nbsp;&nbsp;
...
<!--th>Action</th-->
      ...
      $assets_sql=$con->query("SELECT a.*,m.*,a.id as aid FROM `assets_form_detail` a INNER JOIN assets_master m ON (a.asset_name = m.name) where a.asset in ('Internal Asset')");
	   $i=1;
