...
$stmtc = $con->prepare("SELECT id as cid,enquiry_id FROM cost_sheet_entry WHERE enquiry_id='$cost_id'");
...
$stmtc = $con->prepare("SELECT * FROM cost_sheet_entry WHERE id='$sheet_id'");
					   
					   
$stmtc->execute(); 
$rowc = $stmtc->fetch();

 $enquiry_id= $rowc['enquiry_id'];
 
 $quote_type= $rowc['quote_type'];
 $cost_sheetz_no= $rowc['cost_sheet_no'];
 
 $stmtd= $con->query("SELECT cost_sheet_no,enquiry_id from cost_sheet_entry where cost_sheet_no='$cost_sheetz_no'");
$stmtd->execute(); 
$rowd        = $stmtd->fetch();
$enquiry_id=$rowd['enquiry_id'];

$stmtg= $con->query("SELECT MAX(id) AS cmax,cost_sheet_no,enquiry_id from cost_sheet_entry where enquiry_id='$enquiry_id'");
$stmtg->execute(); 
$rowg        = $stmtg->fetch();

$costi_sheet_no=$rowg['cmax'];

$stmtf= $con->query("SELECT cost_sheet_no,enquiry_id,net_amt,grand_amt,costsheet_date,mar_per,mar_amt from cost_sheet_entry where id='$costi_sheet_no'");
$stmtf->execute(); 
$rowf        = $stmtf->fetch();

$cost_sheet_no=$rowf['cost_sheet_no'];

 $mar_per=$rowf['mar_per'];
$mar_amt=$rowf['mar_amt'];

 $net_amt=$rowf['net_amt'];
...
<div class="card card-primary">
        ...
        <div class="card-body">
	      <div class="form-group row">
		    <div class="col-sm-3"><b>Product/Service/Solution </b>
			     <input type="hidden" class="form-control" id="enquiry_idflow" name="enquiry_idflow" value="<?php echo $cost_id; ?>" readonly>
				  <input type="text" class="form-control" id="pro_ser_ids" name="pro_ser_ids" value="<?php echo $Product; ?>" readonly>
				  <input type="hidden" class="form-control" id="pro_ser_id" name="pro_ser_id" value="<?php echo $value; ?>" readonly>
				 
			     <!--<input type="text" class="form-control" id="pro_ser_id" name="pro_ser_id" value="< ?php echo $pro_ser_type; ?>- < ?php echo $row['name'];?>" readonly>-->
			     <input type="hidden" class="form-control" id="mapping_id" name="mapping_id" value="<?php echo $Product; ?>" readonly>
			...
			<div class="col-sm-2 "><b>Call Type</b>
			<input type="text" class="form-control" id="Call_types" name="Call_types" value="<?php echo $call_types; ?>" readonly>
			<input type="hidden" class="form-control" id="Call_type" name="Call_type" value="<?php echo $call_type; ?>" readonly>
			<input type="hidden" class="form-control" id="cost_sheetz_no" name="cost_sheetz_no" value="<?php echo $cost_sheet_no; ?>" readonly>
			</div>
			...
			<div class="col-sm-2"><b>Quote Currency Type</b>
				<select class="form-control" id="quote_type" name="quote_type" required> <!--onchange="showDiv(this)"-->
                    ...
                    <input type="button" class="delete-row btn btn-danger button" value="Delete" style="float:right;"/>&nbsp;&nbsp;&nbsp;&nbsp;
 ...
 <table class="table table-striped table-bordered table-hover display nowrap"  id="new_tab" style="width:300%" border="1" style="border-collapse:collapse;margin-bottom: 0px !important;" class="table table-bordered">
	 <!--table class="table table-striped table-bordered table-hover display nowrap"  id="new_tab" style="width:100%"-->
		 ...
		 <?php  
				$queryes= $con->query("SELECT * from cost_sheet_entry where cost_sheet_no ='$cost_sheet_no'");
//echo "SELECT * from cost_sheet_entry where cost_sheet_no ='$cost_sheet_no'";			
				 $cnt=1;
					while($quote = $queryes->fetch(PDO::FETCH_ASSOC)){
						$name=$quote['product_name'];
						$product_id=$quote['product_id'];
						$description=$quote['description'];
						$qty=$quote['qty'];
						$unit_rate=$quote['unit_rate'];
						$sel_price=$quote['sel_price'];
						$log_per=$quote['log_per'];
						$dist_per=$quote['dist_per'];
						$dist_amt=$quote['dist_amt'];
						$log_amt=$quote['log_amt'];
						$eng_per=$quote['eng_per'];
						$eng_amt=$quote['eng_amt'];
						$log_eng_amt=$quote['log_eng_amt'];
						$com_per=$quote['com_per'];
						$com_amt=$quote['com_amt'];
						...
						$igst_amt=$quote['igst_amount'];
						$total_gst=$quote['total_gst'];
						$vendor_id=$quote['vendor_id'];
						
						$stmtp = $con->prepare("SELECT id,vendor_name FROM doller_vendor_mastor where id='$vendor_id'");
					   
					   	
			$stmtp->execute(); 
			$rowp = $stmtp->fetch();
			$vendor_name=$rowp ['vendor_name'];
		...
		?>
         <tr>
			 ...
			 if($cnt==1){
								 ...
								 ?>
								 <input type="checkbox" class="messageCheckbox" name="chk[]">
								 <?php
							 }
							 ?>
			 ...
			 <td>
			 <input type="text" class="form-control" onchange="prodcutname(<?php echo $cnt;?>,this.value);desname(<?php echo $cnt;?>,this.value)"  list="clientz_name" autocomplete="off" id="product_name<?php echo $cnt; ?>" value="<?php echo $name; ?>" name="product_name[]" placeholder="select Prodcut Name">
		 <datalist id="clientz_name">
		 <?php 
		$query = $con->query("SELECT name,hsn_code FROM product_master where name !='$name'");
		while ($row_fetch = $query->fetch()) {?>
        ...
        <?php } ?>
		     ...
		     <td>	
 <select name="product_id[]" id="product_id<?php echo $cnt; ?>" class="form-control">
							<option value="<?php echo $product_id; ?>"><?php echo $product_id; ?></option>
					 </select>			 
				 </td>
				 <td>
					 <select name="description[]" id="description<?php echo $cnt; ?>" style="height: 200px; width:300px;white-space: normal;" class="form-control">
		     ...
		     <td>
			     <input type="text" id="qty<?php echo $cnt; ?>" name="qty[]" value="<?php echo$qty;?>" style="width:100%" onchange="totalIt()"onchange="grandtotal();totalIt();" class="form-control" ></td>
		     <!--td>
			     <input type="text" id="unit1" name="unit[]" style="width:100%" class="form-control" placeholder="eg.Nos,Box "></td-->
		     <td>
			     <input type="text" id="cost<?php echo $cnt; ?>" value="<?php echo$unit_rate;?>" name="cost[]" style="width:100%" onchange="totalIt()" class="form-control" ></td>
		     <td>
			     <input type="text" id="price<?php echo $cnt; ?>" value="<?php echo$total_price;?>" name="price[]"  style="width:100%" onchange="totalIt()" readonly value="0.00" class="form-control">
		     </td>
			 <td>
		         <INPUT type="text" id="dist_per<?php echo $cnt; ?>" value="<?php echo$dist_per;?>" name="dist_per[]" style="width:100%" class="form-control dist_per"  onchange="totalIt()" placeholder="%">
		     </td>
		     <td>
		         <INPUT type="text" id="dist_amt<?php echo $cnt; ?>" value="<?php echo$dist_amt;?>" name="dist_amt[]" style="width:100%" class="form-control"  placeholder="0.00" readonly>
		     </td>
			  <td>
		         <INPUT type="text" id="com_per<?php echo $cnt; ?>" value="<?php echo$com_per;?>" name="com_per[]" style="width:100%" class="form-control com_per"  onchange="totalIt()" placeholder="%">
		     </td>
		     <td>
		         <INPUT type="text" id="com_amt<?php echo $cnt; ?>" value="<?php echo$com_amt;?>" name="com_amt[]" style="width:100%" class="form-control"  placeholder="0.00" readonly>
		     </td>
		     <td>
			 <INPUT type="text" id="sel_amt<?php echo $cnt; ?>" value="<?php echo$sel_price;?>" name="sel_amt[]" style="width:100%" class="form-control"  placeholder="0.00" readonly>
		     </td>

		     <td>
			     <input type="text" id="log_per<?php echo $cnt; ?>" value="<?php echo$log_per;?>" name="log_per[]" style="width:100%" class="form-control log_per " onchange="totalIt()" placeholder="%" >
		     </td>
		     <td>
			     <input type="text" id="log_amt<?php echo $cnt; ?>" value="<?php echo$log_amt;?>"name="log_amt[]" style="width:100%" class="form-control"  placeholder="0.00" readonly>
		     </td>		
		     <td> 
		         <INPUT type="text" id="eng_per<?php echo $cnt; ?>" name="eng_per[]" value="<?php echo $eng_per;?>" style="width:100%" class="form-control eng_per"  onchange="totalIt()" placeholder="%">
		     </td>		  
		     <td>
		         <INPUT type="text" id="eng_amt<?php echo $cnt; ?>" name="eng_amt[]" value="<?php echo $eng_amt;?>" style="width:100%" class="form-control " placeholder="0.00" readonly>
		     </td>	
			<td>
		         <INPUT type="text" id="col_item<?php echo $cnt; ?>" value="<?php echo $total_amt;?>" name="col_item[]" style="width:100%" class="form-control"  placeholder="0.00" readonly>
		     </td>
             <td> 
		         <!--<INPUT type="text" id="gst_per1" name="gst_per[]" style="width:100%" class="form-control"  onchange="grandtotal()" placeholder="%">-->
				  <select class="form-control" id="gst_per<?php echo $cnt; ?>" name="gst_per[]" onchange="grandtotal();totalIt();" style="float:left; width: 100%" required>
			          <option value="<?php echo $gst_per;?>"><?php echo $gst_per;?></option>
		     ...
		     <td>
		         <INPUT type="text" id="gst_amt<?php echo $cnt; ?>" value="<?php echo $gst_amt;?>" name="gst_amt[]" style="width:100%" class="form-control " placeholder="0.00" readonly>
		     </td>	
			<td> 
		         <INPUT type="text" id="igst_per<?php echo $cnt; ?>" value="<?php echo $igst_per;?>" name="igst_per[]" style="width:100%" class="form-control"  onchange="grandtotal();totalIt();" placeholder="%" required>
		     </td>		  
		     <td>
		         <INPUT type="text" id="igst_amt<?php echo $cnt; ?>" value="<?php echo $igst_amt;?>"name="igst_amt[]" style="width:100%" class="form-control " placeholder="0.00" readonly required>
		     </td>				 
		     <td>
		         <INPUT type="text" id="tot_item<?php echo $cnt; ?>" value="<?php echo $total_gst;?>" name="tot_item[]" style="width:100%" class="form-control"  placeholder="0.00" readonly>
		     </td>
			  <td align="left">
		     <b><select class="form-control" id="vendor_name<?php echo $cnt; ?>" name="vendor_name[]" style="width:100%;" required>
			...
			<?php } ?>
			  ...
			  <td>
		         <INPUT type="file" id="image<?php echo $cnt; ?>" name="image[]" class="form-control">
		     </td>
...
<?php $cnt++; } ?>		  
      ...
      <table id="dataTable4" width="300px" border="1" style="border-collapse:collapse;" class="table table-bordered" >
	      <tr>
		     <td colspan="3" align="center"><b>Total Margin Percentage%</b></td>
		 
		     <td align="right">
		         <INPUT type="text" id="mar_pper" name="mar_pper" value="<?php echo $mar_per;?>" class="form-control" style="width:58% !important;" readonly>
		    </td>
			<td colspan="3" align="center"><b>Total Margin Amount</b></td>
		 
		     <td align="right">
		         <INPUT type="text" id="mar_aamt" name="mar_aamt" value="<?php echo $mar_amt;?>" class="form-control" style="width:58% !important;" placeholder="0.00" readonly>
		    </td>
		  </tr>
		  <tr>
		     ...
		     <td colspan="3" align="right">
		         <INPUT type="text" id="total_item" value="<?php echo $net_amt;?>" name="total_item" class="form-control" style="width:40% !important;" placeholder="0.00">
		    </td>
		  </tr>
		 ...
		 <tr>
		     ...
		     <td colspan="3" align="right">
		         <INPUT type="text" id="grand_total" value="<?php echo  $grand_amt;?>" name="grand_total" class="form-control" style="width:40% !important;" placeholder="0.00" readonly>
		     </td>
		 </tr>
	 ...
	 <table id="dataTable2" width="300px" border="1" style="border-collapse:collapse;" class="table table-bordered" >
	          <tr>
                   <td><b>Cost Date</b></td>
                   <td colspan="5"><input type="text" value="<?php echo $newDate;?>" style="float:left; width: 50%" class="form-control"  name="chost_date" id="chost_date" required></td>
	  ...
	  <?php
		  $bank = $con->prepare("select a.cost_sheet_no,a.status,b.cost_sheet_no,b.bank_name,b.account_no,b.ifsc_code,b.acc_holder_name,b.branch_name from cost_sheet_entry a left join terms_and_condition b on 
							(a.cost_sheet_no = b.cost_sheet_no) where a.status =3");
							$bank->execute(); 
						    $bank_fetch = $bank->fetch();
		  ?>
	 <br>
	   ...
	   <tr>
         ...
         <td colspan="4"><input type="text" style="float:left; width: 40%" value="<?php echo $bank_fetch['acc_holder_name']; ?>" name="acc_hold_name" id="acc_hold_name" class="form-control" readonly></td>
		 
       </tr>
	   <tr>
         <td><b>BANK NAME :</b></td>
         <td colspan="4"><input type="text" value="<?php echo $bank_fetch['bank_name']; ?>" style="float:left; width: 40%" name="bank_name" id="bank_name" class="form-control" readonly></td>
		 
       </tr>
	   <tr>
         <td><b>BRANCH NAME :</b></td>
         <td colspan="4"><input type="text" value="<?php echo $bank_fetch['branch_name']; ?>" style="float:left; width: 40%" name="branch_name" id="branch_name" class="form-control" readonly></td>
		 
       </tr>
	   <tr>
         <td><b>CURRENT A/C NO:</b></td>
         <td colspan="4"><input type="text" value="<?php echo $bank_fetch['account_no']; ?>" style="float:left; width: 40%" name="account_no" id="account_no"class="form-control" readonly></td>
		 
       </tr>
	   <tr>
         <td><b>IFSC CODE :</b></td>
         <td colspan="4"><input type="text" value="<?php echo $bank_fetch['ifsc_code']; ?>" style="float:left; width: 40%" name="ifsc_code"  id="ifsc_code" class="form-control" readonly></td>
...
$('.wage_content').html('<br><div style="text-align: center;"><img src="qvision/images/images/load3.gif"></div>');	  
           $.ajax({  
                url: 'qvision/BusinessProcess/quotation/quotation_revise_insert.php',
                method:"POST",  
                data:formData, 
				cache: false,
				contentType: false,
...
var margi=(sum2-sum1)/iLen;	

var margix=sum2-sum1;
//alert(margix)	
var profit=(margix/sum1);
//alert(profit)
	 ...
	 document.getElementById("mar_aamt").value = isNaN(margix) ? "0.00" : margix.toFixed(2); 
...
$(".button").on("click", function() {
 ...
 var grid = document.getElementsByName("chk[]");
 ...
 if(j==1){
	 return false();
	    ...
	    var checkBoxes = parseFloat(document.getElementById("tot_item" + j).value);
	  

	   var grand_amt = parseFloat(document.getElementById("total_item").value);

	   
	   var minus=(grand_amt-checkBoxes);
	  ...
	  document.getElementById("total_item").value = isNaN(min) ? "0.00" : min.toFixed(2);
	  document.getElementById("grand_total").value = isNaN(min) ? "0.00" : min.toFixed(2);
  ...
  function deleteRow(new_tab) {
    ...
    try {
      ...
      for (var i = 1; i < rowCount; i++) {
        ...
        var row = table.rows[i];
        var chkbox = row.cells[i].childNodes[i];
alert(chkbox)
	  if (null != chkbox) {
		  alert(i)
		   if(undefined == chkbox.checked){

          table.deleteRow(i);

         rowCount--;
          i--;
	  }else if( true == chkbox.checked){
		  alert(i)
		   table.deleteRow(i);
...
//alert(from_date)
			...
			//alert(from_date)
			  $.ajax({
				  url: "qvision/CRM/find_services.php?Product="+v,
                   type: "GET",
					success: function(result){
						alert(result);
						//$("#product_name"+v).val(data);
						$("#product_name"+v).html(result);
					}
					});
 }  */
