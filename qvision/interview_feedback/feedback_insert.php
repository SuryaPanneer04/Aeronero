...
$sfetch=$staf->fetch();
$staid=$sfetch['id'];//Employee Id
$id=$_REQUEST['id']; //candidate Id
$stmt = $con->prepare("SELECT *,t.rating as overall_rating,c.interview_round_level as round FROM candidate_form_details c left join `technical_team_details` t on c.id=t.candidate_id left join technical_team_commands m on t.id=m.technical_id left join z_department_master d on c.department=d.id left join jobdescription_master dm on c.position=dm.id
WHERE c.id='$id'"); 
...
<form method="POST" action="qvision/interview_feedback/feedback_submit.php">
...
<tr>
<td>Position:</td>
<td colspan="5"><input type="text" class="form-control" id="position" value="<?php echo  $row['tittle'];?>" name="position"readonly></td>
</tr>
...
$sql=$con->query("SELECT s.emp_name as ename,c.candid_id,i.name as depname FROM candidate_round_details c join staff_master s on c.person_id=s.id join interview_rounds i on c.round_id=i.id where c.candid_id='$id' and c.status=3");
...
<option value="5" >Sales level Selected </option>
<option value="7">Sales level Rejected</option>
<option value="8">Accounts level Selected</option>
<option value="9">Accounts level Rejected</option>
</select>
</td>
</tr>
</table>
...
function back()
 ...
 $("#main_content").html(data);
  ...
  var round = $('#round').val()
    ...
    url:'qvision/interview_feedback/feedback_submit.php',
