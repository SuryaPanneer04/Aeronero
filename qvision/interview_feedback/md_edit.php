...
INNER JOIN z_department_master d ON c.department = d.id  join jobdescription_master dm on c.position=dm.id join candidate_round_details r on c.id=r.candid_id
where c.id='$id'"); 
...
<?php
$mdsel=$con->query("select * from md_commands where candidate_id='$id' and md_status='17'");
//echo "select * from md_commands where candidate_id='$id' and status='17'";
$mfet=$mdsel->fetch(PDO::FETCH_ASSOC);
?>
<tr>
<td>CTC :</td>
<td colspan="5"><input type="text" class="form-control" id="ctc" name="ctc" value="<?php echo $mfet['ctc'];?>"></td>
</tr>
<tr>
<td>Feedback :</td>
<td colspan="5"><input type="text" class="form-control" id="remarks_md" name="remarks_md" value="<?php echo $mfet['md_commands']; ?>"></td>
</tr>	
<tr>
<td>Status :</td>
<td colspan="5">
...
<table class="table table-bordered">
<td colspan="6"><input type="submit" class="btn btn-success" style="float:right;" value="Approve"></td>
</table>
</form>
</div>
</div>
