...
<div class="card card-primary">
	...
	if(($userrole !='R096')) {
      ...
      <tr>
		  ...
		  <td>
		  <select name="quote_no" id="quote_<?php echo $cnt; ?>" class="form-control quote_no" style="height:35px;" class="quote_no">
			<option value="">---Select Quote---</option>
			<?php
			

			$datas1=$con->query("select q.id as quote_id,q.quote_no,c.enquiry_id,q.status,c.status from quote_generate q left join cost_sheet_entry c ON q.cost_sheet_no=c.cost_sheet_no  where c.enquiry_id='$enquiry' and (c.status!=1 and c.status!=2)");
			...
			?>
			<option value="<?php echo $data1['quote_id']; ?>"><?php echo $quo;  ?></option>
				<?php } ?>
		  </td>
		  <td><?php 
