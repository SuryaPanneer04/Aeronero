...
$staid=$sfetch['id'];
$id=$_REQUEST['id'];
$stmt = $con->prepare("SELECT * FROM candidate_form_details c left join  z_department_master d on c.department=d.id left join jobdescription_master dm on c.position=dm.id
WHERE c.id='$id'"); 
$stmt->execute(); 
$row = $stmt->fetch();
?>
...
<td colspan="5">
<input type="hidden" class="form-control" id="id" name="cid"  value="<?php echo $id;?>" readonly>
<input type="hidden" class="form-control" id="user_id" name="sid"  value="<?php echo  $staid;?>" readonly>
...
$sql2=$con->query("SELECT interview_rounds.id AS interviewroundid,interview_round_name.id AS intername_id,candidate_round_details.*,interview_rounds.*,interview_round_name.* FROM `candidate_round_details` 
LEFT JOIN interview_rounds ON candidate_round_details.round_id=interview_rounds.id 
LEFT JOIN interview_round_name ON interview_rounds.id=interview_round_name.inter_id 
