...
<?php
$sql=$con->query("SELECT s.emp_name as ename,c.candid_id,i.name as depname,count(*) as salesrow FROM candidate_round_details c join staff_master s on c.person_id=s.id join interview_rounds i on c.round_id=i.id where c.candid_id='$id' and c.status=5 and c.person_id='$userid'");
...
<input type="text" class="form-control" id="technical_department" name="technical_department" value="<?php echo  $rows1['ename']; ?>" readonly></td>
 ...
 <table class="table table-bordered">
<h3><center> Sales Round Feedback Details </center></h3>
<tbody>
...
join interview_round_name r on d.round_name_id=r.id where c.candid_id='$id' and c.status=5 and c.person_id='$userid' and d.hire_id='$userid'");
...
$sql=$con->query("SELECT s.emp_name as ename,c.candid_id,i.name as depname,count(*) as accrow FROM candidate_round_details c join staff_master s on c.person_id=s.id join interview_rounds i on c.round_id=i.id where c.candid_id='$id' and c.status=8 and c.person_id='$userid'");
$accrows = $sql->fetch(PDO::FETCH_ASSOC);
if($accrows['accrow']!=0){
?>
<table class="table table-bordered">
...
<td> Department </td>
<td colspan="2">
<input type="text" class="form-control" id="technical_department" name="technical_department" value="<?php echo  $accrows['depname']; ?>" readonly></td>
</tr>
<tr>
<td>Manager Name</td>
<td colspan="2">
<input type="text" class="form-control" id="technical_department" name="technical_department" value="<?php echo  $accrows['ename']; ?>" readonly></td>
</tr>
</table>

<table class="table table-bordered">
<h3><center> Accounts Round Feedback Details </center></h3>
<tbody>
<?php
$sql=$con->query("SELECT s.emp_name as ename,c.candid_id,i.name as depname,c.*,s.*,i.*,d.*,r.* FROM candidate_round_details c 
join staff_master s on c.person_id=s.id join interview_rounds i on c.round_id=i.id 
JOIN accounts_domain_entries d ON c.candid_id=d.candids_id
join interview_round_name r on d.round_name_id=r.id where c.candid_id='$id' and c.status=8 and c.person_id='$userid' and d.hire_id='$userid'");
