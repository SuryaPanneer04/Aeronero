...
//echo "select a.*,b.emp_name as name,c.designation_name as design,d.dept_name as dept from attire_form a join staff_master b on (a.emp_no=b.candid_id)  join designation_master c on (a.design_id=c.id) join z_department_master d on (a.dep_id=d.id) where (a.date between '$from_date' and '$to_date') group by a.id
//order by a.date desc";
 $cnt = 1;
