...
<?php
	...
	?>
		...
		<input type="text" class="form-control" id="position" name="position" value="<?php echo $fet4['dsgn_name'];?>" readonly>
			...
			<td colspan="5">
            <input type="text" class="form-control" id="client_org_name" name="client_org_name" 
       value="<?php echo isset($fet4['client_org_name']) ? $fet4['client_org_name'] : 'Nill'; ?>" readonly>
			</td>
        </tr> 
       ...
       value="<?php echo isset($fet4['location']) ? $fet4['location'] : 'Nill'; ?>" readonly>
		</tr>
	  <?php } else { ?>
...
<?php
$sqll=$con->query("SELECT s.emp_name as ename,c.candid_id,i.name as depname FROM candidate_round_details c join staff_master s on c.person_id=s.id join interview_rounds i on c.round_id=i.id where c.candid_id='$id' and c.status='16'");
$cnt=1;
$k=0;
...
join interview_round_name r on d.round_name_id=r.id where c.candid_id='$id' and (c.status='16' or c.status='100')");
$cnt=1;
$k=0;
