...
<div  class="card card-primary">
   ...
   <form method="POST" enctype="multipart/form-data">
    ...
    <table class="table table-bordered">
	 ...
	 if($mdstatus=='0' and $finstatus=='0' and $marstatus=='0')
	 {
		 ?>
		 <tr>  
        <td colspan="11"><input type="button" class="btn btn-success" name="save" id="<?php echo $quoteid; ?>" onclick="approve_po(this.id)" style="float:right;margin: 5px;" value="Approve"></td>
	 ...
	 ?>
		...
		<tr>
		<td >
		<input type="button" name="submit" class="btn btn-success" id="<?php echo $quoteid;?>" onclick="submit_po(9,this.id)" style="float:right" value="Submit">
		</tr>
		</div>
        </table>
...
function back()
	...
	<script>
function submit_po(c,v)
{
	var enquiry_id  = document.getElementById("enquiry_id").value;
	var remark=$('#remarks').val();
	$.ajax({
		type:"post",
		data:"id="+v+"&remark="+remark+"&enquiry="+c,
		url:"qvision/BusinessProcess/po_approval/marketing_po_reject.php?id="+v+"&remark="+remark+"&enquiry="+c,
...
function approve_po(v)
{
	...
	$.ajax({
		...
		url:"qvision/BusinessProcess/po_approval/marketing_po_submit.php?id="+v+'&cost_sheet_no='+cost_sheet_no+'&enquiry_id='+enquiry_id,
		success:function(data)
		{
...
function marketing_po_mail()
    {
        ...
        $.ajax({
            ...
            url: "qvision/BusinessProcess/po_approval/marketing_po_mail_post.php?costsheet_no="+cost_sheet_no1,
            success: function (data) {
