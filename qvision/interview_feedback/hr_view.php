...
$stmt = $con->prepare("select c.status as candidate_status,c.id as candidate_id,c.*,d.*,dm.* from candidate_form_details c left JOIN z_department_master d ON c.department = d.id left join jobdescription_master dm on c.position=dm.id left join candidate_round_details r on c.id=r.candid_id where c.id='$id'"); 
$stmt->execute(); 
$row = $stmt->fetch();
...
<?php
$sqll=$con->query("SELECT s.emp_name as ename,c.candid_id,i.name as depname FROM candidate_round_details c join staff_master s on c.person_id=s.id join interview_rounds i on c.round_id=i.id where c.candid_id='$id' and (c.status='35' || c.status='37')");
...
join interview_round_name r on d.round_name_id=r.id where c.candid_id='$id' and (c.status='35' || c.status='37')");
