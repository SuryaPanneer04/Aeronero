...
.card-primary:not(.card-outline)>.card-header{
	...
	<form method="GET" name="b_to_b_reports" role="form">
	...
	<div class="row">	
		...
		<div class="col-lg-3">
		<div class="form-group">
		    <input type="month" name="b_to_bmonth" class="form-control" required>  
		</div>
		</div>
		
		<div class="col-lg-2">
		<div class="form-group">		
		<input  type="button" class="btn btn-default" value="search" onclick="b_to_b_empview()">
		</div>
		</div>
...
function b_to_b_empview()
{
	var data = $('form').serialize();
	$.ajax({
	type: "GET",
	url: "/ssinfo1/qvision/reports/b_to_b_reports/b_to_b_report_empview.php",
	data: data,
	success: function(data)
	{
	$("#b_to_b_details_view").html(data);		
	}				
	});	
}
</script>