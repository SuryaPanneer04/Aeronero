...
<div id="table_view">
 <div class="card card-primary">
    ...
    <table id="example1" class="table table-bordered">
      ...
      <tbody>
      <?php
	  $emp_sql=$con->query("SELECT *,c.id as cid,c.status as status FROM `candidate_form_details` c  left join jobdescription_master d on c.position=d.id left join z_department_master z on c.department=z.id left join interview_schedule_detail i on c.resource_id=i.resource_id  where c.status=2 or c.status=3 or c.status=4 or c.status=6 or c.status=20 or c.status=5 or c.status=7 or c.status=8 or c.status=9 or c.status=13 or c.status=14 or c.status=15 or c.status=16 or c.status=17 or c.status=18 or c.status=19 or c.status=20 or c.status=22 or c.status=23  or c.status=30 or c.status=35 or c.status=37 or c.status=40 order by c.id desc");
      $i=1;
      while($emp_res = $emp_sql->fetch(PDO::FETCH_ASSOC))
      {
      $emp_id = $emp_res['id'] ;
	  $cid = $emp_res['cid'] ;
	  $cstatus = $emp_res['status'];
	  
	  $correct_sta=$con->query("SELECT c.round_id,i.name,c.candid_id,f.first_name,c.status as canstatus FROM `candidate_round_details` c left join interview_rounds i on c.round_id=i.id join candidate_form_details f on c.candid_id=f.id where f.id='$cid' and c.status=3 order by  c.id desc limit 1");
...
if(($emp_res['status']== 9))  
{
echo '<span style="color:green;text-align:center;"><b>Accounts level rejected</b></span>';
}
if(($emp_res['status']==5))  
{
...
?>
	...
	<?php 
		...
		<?php	  
		 ...
		 else if( $emp_res['status'] == 5 || $emp_res['status'] == 8 || $emp_res['status'] == 13 || $emp_res['status'] == 16 || $emp_res['status'] == 40)
		  {
			 ?> 
