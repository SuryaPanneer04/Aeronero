...
if($userrole=='R001'){
	...
	$sql=$con->query("SELECT a.id,a.client_org,a.created_by as created_by,a.created_on as created_on,a.client_name,b.feedback,b.feedback_date,b.date,a.status FROM `crm_calls` a left join `crm_calls_feedback` b on  (a.id=b.calls_id) where (a.employee='$candidateid' or a.created_by='$user_id')  order by a.id desc");
	// echo "<pre>";
	// echo "SELECT a.id,a.client_org,a.created_by as created_by,a.created_on as created_on,a.client_name,b.feedback,b.feedback_date,b.date,a.status FROM `crm_calls` a left join `crm_calls_feedback` b on  (a.id=b.calls_id) where (a.employee='$candidateid' or a.created_by='$user_id') group by a.id  order by a.id desc";
	// echo "</pre>";
}
