...
<div  class="card card-primary">
	...
	<h2 class="card-title"><font size="5"><b>Invoice Approve List</b></font> </h2></div>
	<div class="card-body">
    <table id="example1" class="table table-bordered table-striped">
	 ...
	 if($count==1)
{
	$quote=$con->query("select a.id as pur_invoice_id, b.id,b.cost_sheet_no,b.so_number,a.status from purchase_generate a left join purchase_vendor_master b on a.purchase_id = b.id  order by a.id desc");
  ...
  ?>
	...
	<td>  
<?php
	if($quote_list['status'] == 1){
	?>
	<button class="btn btn-info" onclick="invoice_request_edit(<?php echo $quote_list['id']; ?>,<?php echo $quote_list['pur_invoice_id'];?>)"><i class="fa fa-edit"></i></button>
	<?php
	}
	?>
    <button class="btn btn-success right" onclick="invoice_request_view(<?php echo $quote_list['id']; ?>)"> <i class="fa fa-eye"> </i> </button>	
...
<script>
function invoice_request_view(v)
{
	  	$.ajax({
	type:"POST",
  url:"qvision/Purchase_process/delivery_challan/invoiceApprove_view.php?id="+v,
...
function invoice_request_edit(v,id)
{
	$.ajax({
	type:"POST",
    url:"qvision/Purchase_process/delivery_challan/invoiceApprove_edit.php?id="+v+ "&invoice="+id,
	success:function(data)
	{
		$("#main_content").html(data);
	}
